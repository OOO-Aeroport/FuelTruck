...
@app.route('/fueltruck/<int:object_number>/<int:checkpoint_number>/')
def show_checkpoint(object_number, checkpoint_number):
    print(f"Объект: fueltruck, Номер объекта: {object_number}, Контрольная точка: {checkpoint_number}")
    return f"Объект: fueltruck, Номер объекта: {object_number}, Контрольная точка: {checkpoint_number}"
...
receive_thread = threading.Thread(target=CliGame.receive_messages)
